...
(async () => {
    ...
    // CPP comunication
    try {
        await window.electronAPI.runCommand(JSON.stringify({ msg: '5' }));
        ...
        if (finalReply == 'not-found') {
            window.electronAPI.alertBox(JSON.stringify({ type: 'warning', title: 'Input Error', msg: 'Booking not found or may be someone already checked in to this room !' }));
